...
async function connectSession(number) {
  ...
  // caller (requestPairingCode) tries to use the socket.
    // Wait for the socket to emit its first event or timeout.
    // We need to be SURE the connection is ready.
    await Promise.race([
      firstUpdate,
      new Promise((resolve) => setTimeout(resolve, 20000))
    ]);
    
    // Extra pause to ensure internal Baileys state is settled
